    ...
    def collect_with_network(self) -> tuple[list[ContainerMetrics], int, int]:
        try:
            ...
            with ThreadPoolExecutor() as executor:
                results = list(executor.map(self._collect_single, client.containers.list(all=True)))
            return [r[0] for r in results if r[0] is not None], sum(r[1] for r in results), sum(r[2] for r in results)
        except Exception as e:
